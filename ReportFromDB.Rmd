...
Combination hydrocodone-acetaminophen accounted for `r sum(stats_opioids[1, 2])` opioid claims (`r stats_opioids[1, 3] %>% unlist(use.names = FALSE)` %), oxycodone acetaminophen `r sum(stats_opioids[2, 2])` opioid claims (`r sum(stats_opioids[2, 3])`%), followed by tramadol hydrochloride `r sum(stats_opioids[3, 2])` opioid claims (`r sum(stats_opioids[3, 3])` %), and lastly oxycodone `r sum(stats_opioids[4, 2])` opioid claims (`r sum(stats_opioids[4, 3])`%).
...
`r cbind(t(claims_10pp), t(claims_5pp))`
