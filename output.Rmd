...
#current_working_dir <- dirname(rstudioapi::getActiveDocumentContext()$path) #errors out with knitr
setwd(current_working_dir)
batch <- as.integer(Sys.time())
 #
db <- '/Volumes/Video Projects Muffly 1/Opioids/PUF.db'
...
rmd_total_fpmrs <- opioidRX %>% select(NPI) %>% unique() %>% ungroup() %>%  #number of urogyns
  count() %>% unlist(use.names = FALSE)

rmd_fpmrs_data <- opioidRX %>% group_by(NPI) %>% #not working
  summarise(`Opioid Claim Count` = round(mean(`Opioid Claim Count`),0)) %>% ungroup() %>% summarise(
  LE10 = sum(`Opioid Claim Count` <= 10), #LE = less than or equal to 10
  GT10 = sum(`Opioid Claim Count` > 10)) %>% #GT10 is greater than 10 
...
fpmrs_GE10 <- opioidRX %>% group_by(NPI) %>% 
  ...
  unlist(use.names = FALSE)  #NPI number of FPMRS who prescribed more than 10 opioids
...
```{r query_1}
fpmrs_GE10_PUF_13 <- PUF_13 %>% filter(NPI %in% fpmrs_GE10) %>% collect() #GE stands for greater than or equal to 10
...
#Outputs total number of beneficiaries and the total number of opioid claims written by FPMRS
...
Not correct: `r round(total_count$total_claim_count, 0)`
`r round(total_count$total_bene_count, 0)`
`r round(stats_claims$mean_claim, 0)` (`r round(stats_claims$sd_claim, 0)`

Among FPMRS prescribing at least 10 opioid claims in the Part D Prescriber PUF, a total of 
120,870 opioid claims were prescribed to 96,890 beneficiaries (Table 1).  Each FPMRS prescribed to a mean (SD) of ) opioid claims to a mean of `r round(stats_claims$mean_bene, 0)` (`r round(stats_claims$sd_bene, 0)`) beneficiaries.  
