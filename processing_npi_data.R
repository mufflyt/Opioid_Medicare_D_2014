...
invisible(gc())
NPPES <- read.csv("Data/NPI/npidata_pfile_20050523-20200913.csv",stringsAsFactors = FALSE, colClasses = cls) 
invisible(gc())
...
NPPES1 <- NPPES %>%
  ...
  readr::write_rds("Data/NPI/npidata_pfile_20050523-20200913_cleaned.rds")
...
library(fs)
fs::dir_ls()
file_paths <- fs::dir_ls("~/Dropbox (personal)/Opioid_Medicare_D_2014/Data/Drugs") #Location of the PUF Drug files downloaded, PartD_Prescriber_PUF_NPI_Drug_13.txt
...
#Top 10 Each beneficiatry received a mean of x opioid claims
...
###########################
continue_opioids_past_one_year <- format(round(beneficiaries * 0.051, digits = 0), big.mark=",") #Table 3 Opioid use at 1 y, %
continue_opioids_past_one_year

continue_opioids_paste_three_year_low <- format(round(beneficiaries * 0.024, digits = 0), big.mark=",") #Table 3 Opioid use at 1 y, %
continue_opioids_paste_three_year_low

continue_opioids_paste_three_year_high <- format(round(beneficiaries * 0.053, digits = 0), big.mark=",") #Table 3 Opioid use at 1 y, %
...
leftover_opioid_pills <- format(round(sum(drugs_all_years_fpmrs_only$total_claim_count) * 0.054, digits = 0), big.mark=",") #Leftover pills, 5.4% No. of leftover pills per claim
